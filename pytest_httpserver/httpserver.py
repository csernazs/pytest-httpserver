...
class HTTPServer(HTTPServerBase):  # pylint: disable=too-many-instance-attributes
    ...
    def get_matching_requests_count(self, matcher: RequestMatcher) -> int:
        """
        Queries to log for matching requests, returning the number of log
    ...
    def assert_request_made(self, matcher: RequestMatcher, *, count: int = 1):
        ...
        specified. The expected count can be specified for different checks
        (including zero, which asserts that no requests made for the given matcher).

        :param matcher: the matcher object to match requests
        :return: ``None`` if the assert succeeded, raises :py:class:`AssertionError` if not.
        """
